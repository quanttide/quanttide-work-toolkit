...
工作区是建模单位，**不是目录**——它只装内容（装载起来的定义与任务），不装位置。物理位置由平台给：落点与占位要用的目录基准，当参数传进来。落点（产物落哪）、流水判定（走过哪几步）、定义核对三件事跨着定义与现场，都是工作区的操作。
...
落点与占位的第二个参数是**平台给的目录基准**。四个占位与落点同源：`{{report}}` / `{{journal}}` / `{{log}}` 换的是这三样产物的落点，`{{artifacts}}` 换的是产物目录——于是判据里写占位与直接写落点等价。
