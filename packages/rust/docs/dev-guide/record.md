...
- `models.rs`——账上的一笔：`WorkRecord` 七字段（`id` / `seq` / `created_at` / `order_id` / `step_id` / `description` / `is_succeeded`），读出即校验——`from_jsonl` 认 JSONL 行，构造与语法校验一次完成（字段表之外拒、必选缺失拒、类型不符拒）；`to_jsonl` 落形为单行 JSON。
...
1. `models.rs`：七字段、读出即校验（`from_jsonl`）、`to_jsonl` 落形；
