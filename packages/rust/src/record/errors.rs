...
impl RecordError {
    pub fn new(ordinal: usize, fault: Fault) -> Self {
        RecordError { ordinal, fault }
    }

    /// canonical 报错文字：`{file} 第 n 笔{这一条}`；文件由端侧在渲染时给。
    pub fn message(&self, file: &str) -> String {
        format!("{file} 第 {} 笔{}", self.ordinal, self.fault.text())
    }
}

impl std::fmt::Display for RecordError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "第 {} 笔{}", self.ordinal, self.fault.text())
    }
}
...
mod tests {
    ...
    #[test]
    fn message_prefixes_the_file_and_keeps_the_ordinal() {
        let error = RecordError::new(3, Fault::BadSeq);
        assert_eq!(
    ...
    fn unknown_fields_are_named_in_the_text() {
        let error = RecordError::new(1, Fault::UnknownFields(vec!["step".into()]));
        assert_eq!(
            error.message("records.jsonl"),
